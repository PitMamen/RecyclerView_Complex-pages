...
public class MainActivity extends AppCompatActivity implements RefreshUtils.RefreshListenser {
    ...
    RecyclerView rvMainactivity;  //RecyclerView 控件
    @Bind(R.id.mrl_mainactivity)
    MaterialRefreshLayout mrfMainactivity;  //刷新控件
    private RefreshUtils refreshUtils;
    private Context mContext;
    private HomepagerRecycleAdapter homepagerRecycleAdapter;  //adapter
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        ...
                        if (headerbean != null) {
                            ...
                            if (headerbean.getData().size() != 0) {
                                homepagerRecycleAdapter.setheaderbean(headerbean);  //获取数据
        ...
        OkGo.get(Contants.API.CAMPAIGN_HOME)
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        s = "{" + "\"data\":" + s + "}";
                        Gson gson = new Gson();
                        RefreshBean refreshBean = gson.fromJson(s, RefreshBean.class);
                        if (refreshBean != null) {
                            if (refreshBean.getData().size() != 0) {
